...
BuildRequires: %{?scl_prefix}expat-devel >= 2.1.0
...
# yet upgraded expat:
Requires: %{?scl_prefix}expat >= 2.1.0
...
LD_LIBRARY_PATH="$topdir/$ConfDir:$LD_LIBRARY_PATH" PATH=$PATH:$topdir/$ConfDir make -s EXTRA_CFLAGS="$CFLAGS" %{?_smp_mflags}
...
%config(noreplace) %{?scl:%_root_sysconfdir}%{!?scl:%_sysconfdir}/rpm/macros.python2%{?scl:.%scl}
